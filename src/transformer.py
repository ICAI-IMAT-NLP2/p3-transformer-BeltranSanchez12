...
class Transformer(nn.Module):
    ...
    def __greedy_decode(self, src_input: torch.Tensor, max_length: int, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token = logits.argmax(dim=-1).unsqueeze(1)  # (batch_size, 1)
    ...
    def __beam_search_decode(self, src_input: torch.Tensor, max_length: int, beam_size: int = 3, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                log_probs = nn.functional.log_softmax(logits, dim=-1) #Shape: (1, vocab_size)
    ...
    def __sampling_decode(self, src_input: torch.Tensor, max_length: int, temperature: float = 1.0, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            scaled_logits = logits /temperature # tensor / scalar
            ...
            print("next_token:", next_token.squeeze().tolist())
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            topk_probs = topk_log_probs / topk_log_probs.sum(dim=-1, keepdim=True) # Normalize
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            sorted_probs = F.normalize(sorted_probs, dim = -1)            # Sample from the filtered distribution
            # reconstruir distribución en espacio vocab
            filtered_probs = torch.zeros_like(probs).scatter(1, sorted_indices, sorted_probs)

            # renormalizar
            filtered_probs = F.normalize(sorted_probs, dim = -1)
            # muestrear directamente en vocab space
            next_token = torch.multinomial(filtered_probs, num_samples=1)  # (batch, 1)
            # Map sampled indices to original token indices
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            probs = F.softmax(logits, dim = -1)
            # Get the top k tokens
            topk_log_probs, topk_indices = torch.topk(probs, k, dim=-1)
            topk_probs = topk_log_probs / topk_log_probs.sum(dim=-1, keepdim=True) # Normalize
            ...
            best_token = topk_indices[torch.arange(batch_size, device=device), best_idx].unsqueeze(1)  # (batch, 1) ?? # Shape: (1, 1)
